function res = function_to_minimize_v3(Xn, previous_conditions, dt, contact_points, settings)
    % Returns the function to be minimized for the Newton-Raphson Method
    % This is the linear version, only B1
    ...
    %if theta_contact < pi
        ...
        R7 =  (sum(coefs .* [previous_COM_vel, center_of_mass_vel], 2) - ...
            dt * (-1/Fr + (contact_points > 0) * current_pressures(2))); % + sum(current_deformation .* Xl))); % + current_pressures(2) = south-pole based
