function tests = function_to_minimize_v3_test
...
function setupOnce(testCase)
    ...
    testCase.TestData.current_conditions = ProblemConditions_v3( ...
        testCase.TestData.M, ...
        zeros(M, 1), ...
        zeros(M, 1), ...
        zeros(M+1, 1), ...
...
function testJacobian(testCase)
    ...
    L = length(Xn);
    ftm = @(Xn) function_to_minimize_v3(Xn, previous_conditions, dt, contact_points, testCase.TestData.settings);
    actual_value = zeros(length(ftm(Xn)), length(Xn));
    COL = ftm(Xn);
    ddx = testCase.TestData.ddx;
    for jj = 1:L
        perturb = zeros(L, 1); perturb(jj) = ddx;
        actual_value(:, jj) = (ftm(Xn + perturb) - COL)/ddx;
    end
    expected_value = JacobianCalculator_v3(Xn, previous_conditions, dt, contact_points, testCase.TestData.settings);
...
function testEqualityJacobian(testCase)
    % No contact points, zero initial conditions, zero testing value
    dt = 1;
    ...
    %initial conditions
    Xn  = [rand(M-1, 1); rand(M-1, 1); rand(M+1, 1); rand(); rand()];
    Xn2 = [rand(M-1, 1); rand(M-1, 1); rand(M+1, 1); rand(); rand()];

    expected_value  = JacobianCalculator_v3(Xn,  previous_conditions, dt, contact_points, testCase.TestData.settings);
    expected_value2 = JacobianCalculator_v3(Xn2, previous_conditions, dt, contact_points, testCase.TestData.settings);
    
    verifyEqual(testCase, expected_value2, expected_value, 'AbsTol', 1e-5); 
end
