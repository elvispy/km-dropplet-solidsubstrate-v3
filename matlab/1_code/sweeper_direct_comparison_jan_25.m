...
simulations_cgs.rhoS(1:length(Ohs)) = rho;
simulations_cgs.sigmaS(1:length(Ohs)) = sigma;
simulations_cgs.g(1:length(Ohs)) = g;
simulations_cgs.undisturbed_radius(1:length(Ohs)) = sqrt(Bos * sigma / (rho *g));
Ros = simulations_cgs.undisturbed_radius;
simulations_cgs.nu(1:length(Ohs)) = Ohs .* sqrt(sigma*Ros/rho);
simulations_cgs.initial_velocity(1:length(Ohs)) = - sqrt(Wes .* sigma./(rho * Ros));
simulations_cgs.harmonics_qtt(1:length(Ohs)) = 90;
simulations_cgs.version(1:length(Ohs)) = 3;
